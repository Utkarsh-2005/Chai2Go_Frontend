...
function capitalizeAllWords(str:string) {
  ...
  return (
    ...
    <div className="flex flex-col border-[1.5px] border-black m-5 p-2 rounded">
    <div className="flex justify-center m-1 mb-3">
      <h2>Order No.</h2>
      <h2>{props.orderno}</h2>
    </div>
    <div className="flex">
      <h2 className='w-[70px]'>Base</h2>
      <div className={`flex justify-center w-[150px] mx-1 ${props.base==='water'? 'bg-blue-400': props.base==='creamy-milk'? 'bg-orange-100': props.base==='skimmed-milk'? 'bg-stone-300': 'bg-transparent'}`}>
      <h2>{capitalizeAllWords(props.base)}</h2>
      </div>
    </div>
    <div className="flex">
      <h2 className='w-[80px]'>Spice</h2>
      <div>
        <h2>
          {props.spice === "[]" ? "None" : (() => {
    ...
    <div className="flex">
      <h2 className='w-[80px]'>Container</h2>
      <p>{props.container.charAt(0).toUpperCase()+props.container.slice(1)}</p>
    </div>
    <div className="flex">
      <h2 className='w-[80px]'>Quantity</h2>
      <p>{props.quantity}</p>
    </div>
    <div className="mt-auto flex justify-center"> {/* This div pushes the button to the bottom */}
      <button type="button" className="focus:outline-none text-white bg-red-400 hover:bg-red-800 font-medium rounded-md text-sm px-5 py-2.5 m-3" onClick={() => setShowModal(true)}>Cancel Order</button>
    </div>
    {/* <p>{props._id}</p> */}
  </div>
  </>
